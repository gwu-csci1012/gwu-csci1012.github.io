...
- In particular: be careful to avoid spaces where they shouldn't be spaces, and to use <em>lowercase</em> when asked. For example:
    - `module0.0.pdf` (correct)
    - `Module0.0.pdf` (wrong: starts with uppercase)
    - `module 0.0.pdf` (wrong: space before 0)
    - `module0.0.docx` (wrong: not a pdf).
...
- Another example from Module 0  in Unit 0: In Exercise 0.4, you are instructed to create the files `error1.py` and `error2.py`. These need to be saved in your `module0.0` folder when using your code editor (_Thonny_ or _Spyder_).
- Every module will also have a single PDF file with the same first-part named as the module name. For example: you will make a file called `module0.0.pdf` in your `module0.0` folder.
- When is the PDF file to be used? The single PDF (one per module) is used when you are to write responses outside of Python code. For example:
    - In Module 0, you are asked to submit a picture and write (in English) something about yourself. You can write in Word, and save to a PDF file called `module0.pdf` in your `module0.0` folder.
    - In Exercise 1.11 of Module 1.1, you are asked to describe an error. You will describe (in plain English) by writing in `module1.1.pdf`. Similarly, Exercise 1.8 asks you to describe the output of a program. Copy-paste the output from Thonny/Spyder into  `module1.1.pdf`. Make sure `module1.1.pdf` is saved to in your `module1.1`folder.
...
- For example, for **Unit 0 : Module 0.1**, you must zip the `module0.1` folder that is inside the `unit0` folder. Since this folder will contain the `module0.1.pdf` file, it will get included in the zip file.
- When it's time to submit, you will upload this zip file (example: `module0.1.zip`) into the _appropriate place in Blackboard_.
