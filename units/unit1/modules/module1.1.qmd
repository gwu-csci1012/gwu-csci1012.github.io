...
print("OK, we're done")
```
:::{.callout-caution appearance="simple" icon=false title="1.1.1 Exercise"}
In `my_ifexample.py`, type up the above and examine the output. Then, just below the print with 'Hey' (and indented 4 spaces) add another print to print anything. Then, change the value of y to 6 and report the output in `module0.pdf`. Submit the program with these modifications.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.2 Exercise"}
Consider this program:</span>
...
print('Done')
```
Try to mentally execute (trace its execution in your mind) and predict the output before typing it up in `my_ifexample2.py` to confirm.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.3 Exercise"}
Type up the above in `my_ifexample3.py` and examine the output. Then, change the value of y to 6. What is the output? Change y to 5. What is the output? Report these in your module pdf.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.4 Exercise"}
Type up the above in `my_ifexample4.py` and examine the output. Then, try y=6 and y=4. Report results in your module pdf.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.5 Exercise"}
Type up the above in `my_ifexample5.py`. Then, try each of x=1, x=2, x=4, x=5. Draw the execution pathways (similar to the picture above) for each case in your module pdf.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.6 Exercise"}
...
:::{.callout-caution appearance="simple" icon=false title="1.1.8 Exercise"}
Type up the above in `my_nestedif.py`. Draw the flow of execution for the following three cases: (1) when a=3, b=4, c=5; (2) when a=3, b=4, c=2; (3) when a=6, b=4, c=5.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.1.9 Exercise"}
In `my_smallest_of_three.py`, modify the above program so that it prints out, appropriately, one of "a is the smallest", "b is the smallest" or "c is the smallest", depending on the actual values of a, b, and c. Try different values of these variables to make sure your program is working correctly.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.11 Exercise"}
In `my_smallest_of_three2.py`, fill in code to identify which of the three variables has the smallest value, depending on the actual values of a, b, and c. Use if-statements with multiple clauses. Try different values of these variables to make sure your program is working correctly.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.13 Exercise"}
Type up the above in `my_boolean2.py`. Then try a=0.5.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.14 Exercise"}
Type up the above in `my_boolean3.py`, then change z to be 6 (same as y). What do you observe?</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.15 Exercise"}
Suppose integer variables `a,b,c,d,e` have values a=1, b=1, c=3, d=4, e=5. Consider the following three expressions:</span>
...
```
Try to evaluate each expression by hand. Then, in `my_boolean4.py`, write up each of these in an if-statement to see if the result matches with your hand-evaluation.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.1.16 Exercise"}
In `my_boolean5.py`, write a program that begins with</span>

```python
a = -3
b = -4
```
and uses conditionals to print out the absolute difference between the two numbers. In the above case, the difference is 1. In the case of a=3, b=4, the difference is also 1. When a=-3, b=4, the difference is 7.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.18 Exercise"}
In your module pdf, trace through the iterations in the above program. Then, in `my_oddeven.py`, modify the above program so that for every number between n and 1, going backwards, the program prints whether it's even or odd, as in:</span>
```python
10 is even
9 is odd
8 is even
...
```
:::{.callout-caution appearance="simple" icon=false title="1.1.20 Exercise"}
Trace through the values of total and k in your module pdf.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.1.21 Exercise"}
Given a list like</span>

```python
A = [-5, 2, 4, -9, 12, 13, -30, -21, -20]
```
we see that 12,13 and -21,-20 are pairs of consecutive numbers. Write a program called `my_consecutive.py`, with loop and a conditional to identify such consecutive pairs and print them. For the above list, the output should be:</span>
```python
...
:::{.callout-caution appearance="simple" icon=false title="1.1.23 Exercise"}
Suppose you are given two lists like</span>
```python
A = [-5, 2, 4, -9, 12, 13, -30, -21, -20]
B = [2, -9, 11, 16, 13]
```
Notice that some elements of A (like 2) also exist in B.In `my_twolist.py`, use the list membership idea to print those elements of A that are also in B. For the above example, the output should be:</span>
```python
...
:::{.callout-caution appearance="simple" icon=false title="1.1.25 Exercise"}
Trace through the iterations in the above program and explain why it does not work.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.26 Exercise"}
Trace through the iterations in the above program and explain the output. Why does the inner loop start with i+1?</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.28 Exercise"}
Type up the above in `my_cointosses.py` and run it a few times to see what you get.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.29 Exercise"}
Type up the above in `my_cointosses2.py` and run it a few times to see what you get.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.30 Exercise"}
Type up the above in `my_cointosses3.py` and run it a few times to see what you get. Then increase the number of trials to 100000 and see. The theoretical answer is 0.0625 (approximately 6% chance there's no heads in 4 tosses).</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.1.31 Exercise"}
In `my_cointosses4.py`, write a program to run a large number of trials of the following experiment: toss a coin 10 times and record a success if you get an equal number of heads and tails.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.33 Exercise"}
Type up the above in `my_dice.py` and see what you get, reporting in your module pdf. Is there a number other than 7 for which the probability is higher?</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.1.36 Exercise"}
...
:::{.callout-caution appearance="simple" icon=false title="1.1.37 Exercise"}
```python
x = 5
y = 6
if x < y
...
:::{.callout-caution appearance="simple" icon=false title="1.1.38 Exercise"}
```python
x = 5
y = 6
if x !< y:
...
:::{.callout-caution appearance="simple" icon=false title="1.1.40 Exercise"}
```python
a = 2.5
...
```
Identify and fix the error in `my_error4.py`.</span>
:::
