...
## 1.2.0 Wait, stop!
Before continuing it is essential to review functions as described in [**Module 0.2 of Unit-0**](../../unit0/modules/module0.2.qmd)
...
:::{.callout-caution appearance="simple" icon=false title="1.2.1 Exercise"}
Please review that module. Now.</span>
:::
...
```
:::{.callout-caution appearance="simple" icon=false title="1.2.2 Exercise"}
Type up the above in `my_func_example.py` Then, just before the j=6 statement, print the value of i.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.3 Exercise"}
What do each of the above two programs print? Type them up in `my_func_example2a.py` and `my_func_example2b.py` to find out.</span>
:::

###### **1.2.4 Video**</span>
{{< video ../../../media/units/unit1/module1.2/u1-mod2.4.mp4 aspect-ratio="21x9" >}}

:::{.callout-caution appearance="simple" icon=false title="1.2.5 Exercise"}
In `my_func_example3.py`, fill in the code in the function below so that the output of the program is:</span>
```python
*****
***
*
```
The partially-written program:</span>
...
:::{.callout-caution appearance="simple" icon=false title="1.2.7 Exercise"}
In `my_func_example4.py`, fill in the code in the function below so that the output of the program is:</span>
...
The partially written program is:</span>
...
The idea is to compute the sum but only of the positive parameters; if neither value is positive, print 0.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.9 Exercise"}
Type up the above in `my_func_example5.py`. Then try sending 5,-6 instead of -5,6. Then try 5,6. In your module pdf, trace through the execution in each case. What does the `padd()` function achieve?</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.10 Exercise"}
In `my_func_example6.py`, complete the code in `sum_up_to()` so that it computes and then returns the sum of numbers from 1 to n (inclusive of 1 and n).</span>
```python
def sum_up_to(n):
...
:::{.callout-caution appearance="simple" icon=false title="1.2.12 Exercise"}
Type up the above in `my_func_example7.py` and report what it prints in your module pdf.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.13 Exercise"}
Type up the above in `my_func_example8.py` and report what it prints in your module pdf.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.14 Exercise"}
In your module pdf, draw such a diagram for the other two function invocations.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.2.15 Exercise"}
In `my_func_example9.py`, complete the code below so that the function, when given three numbers, identifies the two larger ones and returns their sum.</span>
...
## 1.2.6 What can you do with parameter variables?
...
:::{.callout-caution appearance="simple" icon=false title="1.2.17 Exercise"}
Just by reading, can you tell what the above program prints? Then, implement in `my_func_example10.py` and confirm.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.18 Exercise"}
In your module pdf, trace the execution of the above program, including loop iterations. Type up the above in `my_func_example11.py`, including a `print(k)` in the loop to confirm your trace.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.2.19 Exercise"}
In `my_even_numbers.py`, complete the code in the function below so that the even numbers in a list are printed out:</span>
```python
def print_even(A):
...
print_even(A)
```
Write the function so that the output is:</span>
```python
Found even:  6
Found even:  8
```
Recall how to use the <em>remainder</em> operator from the previous module (conditionals and loops).</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.21 Exercise"}
Trace through the above in your module pdf, then type up in `my_odd_list.py` to confirm. Include a `print(k)` in the loop after the append.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.22 Exercise"}
...
:::{.callout-caution appearance="simple" icon=false title="1.2.23 Exercise"}
In `my_last_negative.py`, modify the above program to find the last negative number (if one exists) in a list. So, after completing the code below</span>
```python
...
:::{.callout-caution appearance="simple" icon=false title="1.2.25 Exercise"}
Type up the above in `my_first_negative2.py`, execute and observe the output. Then, trace through the execution in your module pdf.</span>
:::
...
## 1.2.9 A fundamental difference between list and basic parameters
Consider this program:
```python
def swapint(a, b):
    temp = a
...
:::{.callout-caution appearance="simple" icon=false title="1.2.26 Exercise"}
Before typing up the above in `my_swap_int.py`, can you guess what the output will be?</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.27 Exercise"}
In your module pdf, draw the three boxes for a, b, and temp, at each step in the function's execution.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.28 Exercise"}
In your module pdf, draw the three boxes for a, b, and temp, at each step in the function's execution.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.29 Exercise"}
In your module pdf, draw the above diagram after each line in the function, showing how the list contents and temp change with each line of execution.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.31 Exercise"}
Consider the following program:</span>
```python
def change_int(a):
    a = a + 1
...
print(B)
```
In your module pdf, trace through the above program, showing boxes for the different variables changing as each line executes. Show these diagrams at the start of each function's execution and just before each function returns.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.33 Exercise"}
Consider the following program:</span>
```python
def decrement(a):
    return a - 1
...
print(subtract(3, 13))
```
Type this up in `my_subtraction.py`. Then in your module pdf, trace the execution step by step. Draw diagrams showing the changing contents of variables a and b. (This exercise is a bit long, but worth doing because it will help your understanding.)</span>
:::

###### **1.2.34 Video**</span>
{{< video ../../../media/units/unit1/module1.2/u1-mod2.34.mp4 aspect-ratio="21x9" >}}

## 1.2.11 More stats via programming
...
:::{.callout-caution appearance="simple" icon=false title="1.2.35 Exercise"}
Consider the following program:</span>
```python
def find_smallest(A):
    smallest = A[0]
...
:::{.callout-caution appearance="simple" icon=false title="1.2.36 Exercise"}
In `my_stats.py` complete the above so that the <em>span</em> of the data is computed and printed. This is the difference between the largest and smallest values. Then, use a table (as you have before) to trace through the execution in your module pdf.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.38 Exercise"}
In `my_stats2.py` complete the code above. You should get as output, approximately</span>
```python
mean = 0.1627272727272727
standard deviation = 4.955716792313685
```
(Real numbers, what can you do?)</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.39 Exercise"}
In `my_stats3.py`, add the above to your code from the earlier exercise so that the output is:</span>
```python
mean = 0.1627272727272727
standard deviation = 4.955716792313685
Found outlier:  -10.5
...
:::{.callout-caution appearance="simple" icon=false title="1.2.40 Exercise"}
Type up the above in `my_factorial.py`. Then, in your module pdf trace through what happens when `factorial(5)` is called. Remove the # symbol so that the print prints.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.42 Exercise"}
Download <a href="../../../media/useful/drawtool.py" download>**drawtool.py**</a> and <a href="../../../media/units/unit1/module1.2/circle_square.py" download>**circle_square.py**</a> and run the latter to get</span>
![](../../../media/units/unit1/module1.2/kepler.png)

Then, change n to n=10. Perhaps experiment with colors. Try to read the functions and figure out, not so much the calculation, as what they're doing.</span>
:::
...
:::{.callout-caution appearance="simple" icon=false title="1.2.43 Exercise"}
```python
def square_it(x):
    y = x * x
...
print(b)
```
Identify and fix the error in `my_error1.py`.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.2.44 Exercise"}
```python
def bigger(x,y)
    if x > y:
        return x
...
print(bigger(4,5))
```
Identify and fix the error in `my_error2.py`.</span>
:::

:::{.callout-caution appearance="simple" icon=false title="1.2.405Exercise"}
```python
def find_smallest(A):
    smallest = 1000
    for k in A:
...
```
Does the above program work? What is the reasoning behind the line `smallest = 1000`? What would go wrong if you removed it altogether? Can you create a list (change the data in B) which will cause the function to fail to find the smallest number. Fix the issue in `my_error3.py`.</span>
:::
