...
However, it is important for you to complete it on your own to understand the working of the program `my_list6.py`. 
...
However, it is important for you to complete it on your own to understand the working of the program `my_list7.py`. 

- To confirm your output, trace through the above code showing the values of i and A[i] at each iteration.
- Confirm with the resutl from your code, `my_list7.py`.
:::
...
print(y)  # Does it affect what's in y?
          # No. y = 5. y retains the value of x it was assigned (x = 5) 
...
- Finally, the values of y and x is printed using `print(y)` and `print(x)`.
- Type up program above in `my_variable_copy.py` and run it. 
...
```python
    ...
    print(B)    # Does it affect what's in list B?
                ...
                # any change in elements of A also 
                # made a corresponding 
                # change in elements of B.
```

:::{.callout-caution appearance="simple" icon=false}
### Exercise 0.11
Now consider another example above that involves copying a list.

The following can be observed in the program: 

- First, the a list of 5 elements is initialized, i.e., A = [1, 4, 9, 16, 25]. 
...
- Type up the above in `my_list_copy.py` nd run it. 
- When you run the program you will notice something different than the prevous exercise 0.10. 
    - Following the line of code where you typed, B = A, the change in the elements of A affected the same change in elements of B. 
...
However, it is important for you to complete it on your own to understand the working of the program `my_total_example1.py`. 
...
However, it is important for you to complete it on your own to understand the working of the program `my_twolist1.py`. 
...
However, it is important for you to complete it on your own to understand the working of the program `my_twolist2.py`. 
...
However, it is important for you to complete it on your own to understand the working of the program `my_listswap.py`. 
...
:::{.callout-caution appearance="simple" icon=false}
### Exercise 0.19
Type the code you just wrote, i.e., `my_list_swap1.py` again, in `my_list_swap2.py`.
...
- The output of `my_list_swap2.py` should be shown below.
...
Type the following program above in `my_other_lists.py`:
...
Notice that the program will print `chutesandladders`. 

Insert spaces in the for-loop so that `chutes and ladders` is printed.
...
Type the above in `my_die_roll2.py` to observe the result. 

**Submit `my_die_roll2.py` with num_trials = 10.**

Try this experiment; you don't need to submit the following part of the exercise:

- Try changing num_trials = 100 and see if the results of aaverage score  is any different from your program with num_trials = 10
...
- In each trial, each player rolls the die twice and adds the two numbers. 
- The final score is the average across all trials. 
- Let your program estimate the average score  when you use a large number of trials (say, num_trials = 1000).  

**Submit `my_die_roll2.py` with num_trials = 1000.**
...
Type up the above in `my_plot_example.py` to see what you get. You will also need to download <a href="../../../media/useful/drawtool.py" download>**drawtool.py**</a> into the same folder.
...
In `my_plot_example2.py` change your code from the earlier exercise to make the scale the same along x and y axes.
...
In `my_function_plot2.py`, add a third function \(h(x) = 2<sup>x</sup>\). 

Try out the following options:

- Create the plot with the scale of x and y axes with same range as [0, 1200] as shown below.
    - `dt.set_XY_range(1,1200, 0,1200)`
- Create the plot with the different ranges for scale of x and y axes
    - Set x-range as [0,12] and the y-range as [0,1200] i.e., `dt.set_XY_range(1,12, 0,1200)`.
    - This should show you how puny quadratic growth is compared to exponential growth. 

**Submit your `my_function_plot2.py` by setting the scale of x and y axes with same range as [0, 100] i.e., `dt.set_XY_range(1,100, 0,100)`**
:::
...
Download <a href="../../../media/units/unit1/module0/functional_art.py" download>**functional_art.py**</a> and run to see a plot like the one shown below. 

![](../../../media/units/unit1/module0/Ex032_pic1.png)

If you examine and understand the program `functional_art.py`, you will see that `random.random()`  and `math.sin()` functions were used to generate the plot above.

Now create a program `my_functional_art.py`, by improving on the program `functional_art.py` and including a `math.cos()` function as your additional decoration on the mathematic art of a plot.
