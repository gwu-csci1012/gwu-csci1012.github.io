...
Now consider 2.56<sup>x</sup> = y.

- Try the number `x = 6.4` in the equation above.
- Use Python to compute the value of `y` including it in the program `my_real_example2.py`
- Check if the value of `y` is approximately `400` by changing the the value of `x` from `6.4`.
- You don’t have to be accurate.
- Submit the program `my_real_example2.py` with value of `x` that in your attempts gets the value of `y` closest to the `400`.
:::
